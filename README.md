...
- Permisos equivalentes a los del proceso objetivo. Para procesos elevados, M@D-Engine también debe iniciarse como administrador.
...
`run.ps1` crea `.venv` si falta, instala el proyecto cuando no es importable y reenvía todos los argumentos a M@D-Engine.
...
# o: botón «Iniciar Memory Lab» en M@D-Engine
```
...
overlay compacto siempre visible. Los atajos son globales, pero M@D-Engine ignora la activación
...
M@D-Engine usa la sesión ya iniciada en una CLI local; no solicita, lee ni guarda claves de API.
...
Después abra `Ajustes → IA`, elija **Antigravity CLI**, **Codex CLI** o **Claude Code**. M@D-Engine
...
M@D-Engine siguen pasando por las 20 herramientas Pydantic, `AppController` y la política local.
...
El script ejecuta todas las puertas, limpia `build/` y `dist/`, y genera `dist\M@D-Engine\M@D-Engine.exe` en modo `onedir`. El mismo binario acepta:

```powershell
dist\M@D-Engine\M@D-Engine.exe
dist\M@D-Engine\M@D-Engine.exe --memory-lab
dist\M@D-Engine\M@D-Engine.exe --no-ai
```
...
- **Acceso denegado:** el proceso tiene mayor integridad. Cierre M@D-Engine y reinícielo como administrador; la aplicación no solicita UAC ni se autoeleva.
- **Proceso de 32 bits:** los módulos WOW64 pueden no enumerarse desde M@D-Engine x64. Use direcciones absolutas; el escaneo sigue disponible.
...
- **Qt no encuentra el plugin de plataforma:** reinstale PySide6 en `.venv`; para un build, confirme que existe `dist\M@D-Engine\_internal\PySide6\plugins\platforms\qwindows.dll`.
- **CLI ausente o sin sesión:** ejecute `agy`, `codex login status` o `claude auth status` fuera de M@D-Engine; revise el ejecutable y el modelo en `Ajustes → IA`. La inspección manual sigue operativa con `--no-ai`.
- **El overlay no aparece:** confirme que el proceso adjunto, no M@D-Engine ni otra ventana, está en primer plano; pruebe `Pause` y `Ctrl+Shift+º`. Si otra aplicación reservó un atajo, M@D-Engine lo indica en la barra de estado.
...
4. En M@D-Engine, valor `73`, **F6**. Los candidatos caen a 1 (la dirección coincide con la que muestra el laboratorio).
...
11. Guardar workspace, cerrar M@D-Engine, reabrir, cargar el workspace: las vigilancias vuelven.
...
15. Cerrar la aplicación y comprobar en el Administrador de tareas que no queda ningún proceso `M@D-Engine`/`python` huérfano.
